...
function link(srcRef, dstRef, type, { db = open(), weight, props, actor = 'unknown', reason } = {}) {
...
function unlink(srcRef, dstRef, type, { db = open(), actor = 'unknown', reason } = {}) {
  const src = nodes.getByIdOrUid(srcRef, { db });
  const dst = nodes.getByIdOrUid(dstRef, { db });
