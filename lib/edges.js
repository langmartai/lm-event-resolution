...
function link(srcRef, dstRef, type, opts = {}) {
  const { db = open(), weight, props, actor = 'unknown', reason, sessionId, projectPath, toolUseId, intent } = opts;
  ...
  const txn = db.transaction(() => {
    ...
    updates.record(db, {
      ...
      reason, actor, sessionId, projectPath, toolUseId, intent,
    });
    return edge;
  });
...
function unlink(srcRef, dstRef, type, opts = {}) {
  const { db = open(), actor = 'unknown', reason, sessionId, projectPath, toolUseId, intent } = opts;
  const src = nodes.getByIdOrUid(srcRef, { db });
  const dst = nodes.getByIdOrUid(dstRef, { db });
  ...
  const txn = db.transaction(() => {
    ...
    updates.record(db, {
      ...
      after: null, reason, actor, sessionId, projectPath, toolUseId, intent,
    });
    return true;
  });
