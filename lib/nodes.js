...
function create(input, { db = open(), actor = 'unknown', reason } = {}) {
  validateInput(input);
...
function update(idOrUid, patch, { db = open(), actor = 'unknown', reason } = {}) {
  ...
  const txn = db.transaction(() => {
    ...
    updates.record(db, {
      ...
      changeType, before, after, reason, actor,
    });
    return after;
  });
...
function setStatus(idOrUid, newStatus, { db = open(), actor = 'unknown', reason } = {}) {
  if (!VALID_STATUSES.has(newStatus)) throw new Error(`Invalid status: ${newStatus}`);
  return update(idOrUid, { status: newStatus }, { db, actor, reason });
}

function remove(idOrUid, { db = open(), actor = 'unknown', reason } = {}) {
  const before = getByIdOrUid(idOrUid, { db });
  ...
  const txn = db.transaction(() => {
    ...
    updates.record(db, {
      entityType: 'node', entityId: before.id,
      changeType: 'delete', before, after: null, reason, actor,
    });
    return true;
  });
  return txn();
