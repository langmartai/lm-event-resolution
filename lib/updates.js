...
function record(db, { entityType, entityId, changeType, before, after, reason, actor, sessionId, projectPath, toolUseId, intent }) {
  ...
  const stmt = db.prepare(`
    INSERT INTO updates (entity_type, entity_id, change_type, before_json, after_json,
                         reason, actor, session_id, project_path, tool_use_id, intent)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  return stmt.run(
    entityType,
